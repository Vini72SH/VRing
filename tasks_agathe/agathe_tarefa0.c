...
typedef struct{
	int id; // identificador de facility do SMPL. facility : objeto que está simulando (evento, etc.). aqui o objeto é o processo
} TipoProcesso;
...
int main(int argc, char *argv[]) {
	...
	while (time() < MaxTempoSimulac){
		...
		switch(event) { // " eventos : testes, falha, recovery
			...
			case test: if (status(processo[token].id) != 0) break; // processo falho não testa!
